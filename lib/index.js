...
GoogleAdWordsNew.prototype.page = function(page) {
  ...
  var mappedConversion = {};
  each(function(conversion) {
    // to prevent common casing mistakes in our UI
    if (pageName.toLowerCase() === conversion.key.toLowerCase()) {
      mappedConversion.id = conversion.value.id;
      // in case customer has multiple AdWords accounts
      if (conversion.value.accountId) return mappedConversion.override = conversion.value.accountId;
    }
...
GoogleAdWordsNew.prototype.track = function(track) {
  ...
  each(function(conversion) {
    // to prevent common casing mistakes in our UI
    if (eventName.toLowerCase() === conversion.key.toLowerCase()) {
      mappedConversion.id = conversion.value.id;
      // in case customer has multiple AdWords accounts
      if (conversion.value.accountId) return mappedConversion.override = conversion.value.accountId;
    }
